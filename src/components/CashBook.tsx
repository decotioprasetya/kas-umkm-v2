...
export default function CashBook() {
  ...
  const save = async (e: any) => {
    ...
    await supabase.from('cash_book').insert([{ 
      description: form.desc, amount: Number(form.amount), type: form.type, date: form.date 
    }]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <form onSubmit={save} className="bg-white p-6 rounded-2xl shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4 border">
        <input type="text" placeholder="Keterangan" value={form.desc} onChange={e => setForm({...form, desc: e.target.value})} className="border p-3 rounded-xl w-full" required />
        ...
        <button type="submit" className="md:col-span-2 bg-indigo-600 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-700 transition">
          <PlusCircle size={20}/> Simpan Transaksi
        </button>
      </form>
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        {list.map((item: any) => (
          <div key={item.id} className="p-4 border-b flex justify-between items-center hover:bg-slate-50">
            <div><p className="font-bold text-slate-800">{item.description}</p><p className="text-xs text-slate-400">{item.date}</p></div>
